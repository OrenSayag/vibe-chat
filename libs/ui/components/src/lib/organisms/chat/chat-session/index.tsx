...
function Session({
  ...
  return (
    <Box className={cn('px-12', className)}>
      {grouped.map((g) => (
        <SessionDay messages={g} key={g[0].idMessage} />
      ))}
    </Box>
  );
}

function SessionDay({ messages }: { messages: ChatMessage[] }) {
  return (
    <Box className={'pt-2'}>
      ...
      {messages.map((m) => (
        <ChatMessageBox
          ...
          key={m.idMessage}
          className={cn(
            m.type === MessageType.Outgoing && 'ml-auto',
            m.type === MessageType.Incoming && 'mr-auto'
          )}
        />
      ))}
    </Box>
...
function groupByDateToArrays(data: ChatMessage[]): ChatMessage[][] {
